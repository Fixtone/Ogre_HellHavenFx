...
CPlayState::CPlayState(CGraphicsManager* aCGraphicsManager, CInputManager* aCInputManager, CResourceManager* aCResourceManager)
: CBaseState(aCGraphicsManager, aCInputManager, aCResourceManager)
, mRootSceneNode(0)
...
{	
	...
	mCamera->setFarClipDistance(12000.0f);
	...
	//#################Resources
	mCResourceManager->loadPlayStateResources(mTrayMgr);
	//#################Resources
	
	//#################Particles (HHParticles)
	mHHParticlesManager = new CHHParticlesManager(mSceneManager, mCamera, mTrayMgr);
	mHHParticlesManager->initialize();
	//#################Particles (HHParticles)
	...
	//#################Actor Manager
	mActorManager = new CActorManager(mSceneManager, mPhysicsManager->getPhysicsScene(),mPhysicsManager->getRenderSystem());
	//#################Actor Manager
	...
	difSpecMap = "desert_6_diffusespecular.dds";
	normHeightMap = "desert_6_normalheight.dds";
	...
	//#################TransitionAnimator
	mTransitionAnimatorManager = new CTransitionAnimatorManager(mSceneManager);
	//#################TransitionAnimator		
...
bool CPlayState::frameStarted(const Ogre::FrameEvent& evt)
{
	mHHParticlesManager->updateStartFrame(evt.timeSinceLastFrame);
	return true;
}
/** Rendering queue */
...
{ 
	...
	mActorManager->update(evt.timeSinceLastFrame);
	mTransitionAnimatorManager->update(evt.timeSinceLastFrame);
	mHHParticlesManager->update(evt.timeSinceLastFrame);
    return true;
}
...
void CPlayState::finalize()
{
	...
	if(mTransitionAnimatorManager)
		delete mTransitionAnimatorManager;
...
bool CPlayState::keyPressed(const OIS::KeyEvent& e)
{
	...
	switch(e.key)
    {
		...
		case OIS::KeyCode::KC_1:
			mTransitionAnimatorManager->createCameraTransition(mCameraMan,mCamera,mCamera->getPosition(),mCamera->getDerivedDirection(),Ogre::Vector3(1.0f,500.0f,200.f),Ogre::Vector3(1.0f,100.0f,-800.f),10.0f);
		break;	
		case OIS::KeyCode::KC_B:
			{
				...
				bodyDescription.mLinearVelocity = NxOgre::Vec3(0,0,-10);				
				...
				// Create an Entity			
				Ogre::Entity* box = mSceneManager->createEntity("cube.1m.mesh");
				mActorManager->createCActorBomb(box, box_desc, bodyDescription, mCamera->getPosition(), Ogre::Quaternion::IDENTITY);
			}
		break;
