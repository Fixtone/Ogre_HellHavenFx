...
, mTotalSeconds(0.0f)
{
	...
	mCResourceManager = new CResourceManager();
}
...
CGame::~CGame()
{
	...
	if(mCResourceManager)
		delete mCResourceManager;
}
...
bool CGame::Initialize()
{
	...
	{   
		...
		mCResourceManager->initialize(mCGraphicsManager->GetSceneManager());		
		
		//Initialize Game states (FSM) manager
		mCStatesManager->initialize(mCGraphicsManager, mCInputManager, mCResourceManager);
	}
